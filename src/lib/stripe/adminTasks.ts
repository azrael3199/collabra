...
export const createOrRetrieveCustomer = async ({
  ...
  try {
    ...
    try {
      ...
      console.log("Error while trying to create / update customer ", error);
...
export const copyBillingDetailsToCustomer = async (
  ...
  const newAddress: any = { ...address };
  if (address.city === null) newAddress.city = undefined;

  await stripe.customers.update(customer, { name, phone, address: newAddress });
  try {
    await db
      .update(users)
      .set({
    ...
    throw new Error("Could not copy customer billing address");
  }
};

export const manageSubscriptionStateChange = async (
  ...
  try {
    ...
    console.log(
      `Subscription Data inserted / updated [${subscription.id} for user ${uuid}]}`
    );
